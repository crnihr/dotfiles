...
function Update()
	...
	for i = 1, maximumWorkspace do
		...
		if i <= totalWorkspace then
			...
			SKIN:Bang('!SetOption', 'Workspace'..i, 'LeftMouseUpAction', string.rep('[!CommandMeasure WorkspaceWindowSendMessage "SendMessage 16687 2 1"]',i-totalWorkspace)) --Create new workspace
		end
	end
